...
class TimetableLoader {
    func loadTimetable(token: String, completion: @escaping (Result<[DayLessons], Error>) -> Void) {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                ...
                let daysLessons = self.toDayLessons(subjects: result.subjects, teachers: result.teachers, hours: result.hours, days: result.days)
    ...
    func toDayLessons(subjects: [Subject], teachers: [Teacher], hours: [Hour], days: [Day]) -> [DayLessons] {
            let subjectMap = Dictionary(uniqueKeysWithValues: subjects.map { ($0.id.trimmingCharacters(in: .whitespaces), $0.name) })
            ...
            return days.compactMap { day in
                ...
                let lessons = day.atoms.compactMap { atom -> Lesson? in
                    guard let subjectId = atom.subjectId?.trimmingCharacters(in: .whitespaces),
                          ...
                          let hour = hourMap[atom.hourId] else {
                        return nil
                    }
